 ...
 * @author whilein
 */
object RawArgumentsTests {
    fun implementationTests(argumentFactory: (Int) -> RawArguments) = funSpec {
    ...
    class OfArray : FunSpec({
        include(implementationTests {
            RawArguments.fromList((1..it).map(Int::toString).toList())
        })
    })

    class OfList : FunSpec({
        include(implementationTests {
            RawArguments.fromArray(*(1..it).map(Int::toString).toTypedArray())
        })

        test("Factory with incorrect ranges fails") {
            val array = arrayOf("1")

            shouldThrow<IllegalArgumentException> { RawArguments.fromArray(array, 0, 2) }
            shouldThrow<IllegalArgumentException> { RawArguments.fromArray(array, 2, 0) }
            shouldThrow<IllegalArgumentException> { RawArguments.fromArray(array, -1, 1) }
            shouldThrow<IllegalArgumentException> { RawArguments.fromArray(array, 0, -1) }
        }

        test("Factory with correct range doesn't fail") {
            val arguments = RawArguments.fromArray(arrayOf("1", "2"), 1, 1)
